...
class BaseAccuracy:
    ...
    def __call__(self, predictions: tuple[torch.Tensor, torch.Tensor], targets: tuple[torch.Tensor, torch.Tensor], features: torch.Tensor):
        raise NotImplementedError("BaseAccuracy is an abstract class and should not be called.")
...
class MinigridAccuracy(BaseAccuracy):
    ...
    def _calc_agent_acc(self, predictions: torch.Tensor, targets: torch.Tensor, features: torch.Tensor):
        ...
        samples_one_agent_pred = ((predictions[..., 3].view(-1, predictions.shape[1]*predictions.shape[2]) != 0).sum(dim=1) == 1)
        ...
        perc_samples_agent_stay_correct = agent_correct_perc(predictions, targets, samples_stay_mask_, samples_stay_mask.sum())
        ...
        perc_samples_agent_rotated_correct = agent_correct_perc(predictions, targets, samples_rotated_mask_, samples_rotated_mask_.sum())
        ...
        perc_samples_agent_moved_correct = agent_correct_perc(predictions, targets, samples_moved_mask_, samples_moved_mask_.sum())
