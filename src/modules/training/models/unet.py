...
class UNet(nn.Module):
    ...
    def _build_network(self) -> None:
        ...
        self.down_layers = nn.ModuleList([Down(self.hidden_channels[i], self.hidden_channels[i + 1]) for i in range(len(self.hidden_channels) - 1)])
        ...
        # Up
        self.up_layers = nn.ModuleList([Up(self.hidden_channels[i], self.hidden_channels[i - 1]) for i in range(len(self.hidden_channels) - 1, 0, -1)])
