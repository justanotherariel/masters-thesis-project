    ...
    for grid_idx in tqdm(range(1), desc=f"Dataset {dataset_group.name.lower()}"):
        with PDFFileWriter(output_dir, f"sample_eval_{dataset_group.name.lower()}_{grid_idx}.pdf") as writer:
            ...
            for sample_idx in range(grid_index_len):
                ...
                agent_x_obs_dir = x_obs_grid[sample_idx, agent_x_obs_pos[0], agent_x_obs_pos[1], target_ti.observation[3]].item() - 1
                ...
                agent_y_obs_dir = y_obs_grid[sample_idx, agent_y_obs_pos[0], agent_y_obs_pos[1], target_ti.observation[3]].item() - 1
                agent_y_obs_pos = [((agent_y_obs_pos[0].item(), agent_y_obs_pos[1].item()), agent_y_obs_dir)]
                
                agent_pred_obs_pos_tmp = (pred_obs_grid[sample_idx, :, :, target_ti.observation[3]].squeeze() != 0).nonzero()
                agent_pred_obs_pos = []
