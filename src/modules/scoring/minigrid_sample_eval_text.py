    ...
    with TextFileWriter(output_dir, f"sample_eval_{dataset_group.name.lower()}.txt") as writer:
        ...
        for grid_idx in tqdm(range(len(indices)), desc=f"Dataset {dataset_group.name.lower()}"):
            ...
            for sample_idx in range(grid_index_len):
                agent_starting_pos = (x_obs_grid[sample_idx, :, :, target_ti.observation[3]].squeeze() != 0).nonzero()[0]
                agent_starting_dir = x_obs_grid[sample_idx, agent_starting_pos[0], agent_starting_pos[1], target_ti.observation[3]].item()
                ...
                if sample_errors.any():
                    ...
                    for error_idx in range(error_loc.shape[0]):
                        ...
                        true_val = y_obs_grid[sample_idx, error_loc[error_idx, 0], error_loc[error_idx, 1], error_loc[error_idx, 2]]
                        pred_val = pred_obs_grid[sample_idx, error_loc[error_idx, 0], error_loc[error_idx, 1], error_loc[error_idx, 2]]
                        
                        # Print the error
                        writer.new_error((error_loc[error_idx, 0], error_loc[error_idx, 1]), error_type, true_val, pred_val)
...
class TextFileWriter():
    ...
    def new_sample(self, sample_idx: int, action: int, agent_starting_pos: tuple[int, int], agent_starting_dir: int):
        ...
        self.buffer.write(f"Sample [{sample_idx:>4}] - ({agent_starting_pos[0]:>2}, {agent_starting_pos[1]:>2}), {dir_str:>5} | {action_str:>7}\n")
    ...
    def new_error(self, error_pos: tuple[int, int], error_type: str, true_val: int, pred_val: int):
        ...
        self.buffer.write(f"{self._indent}{error_type} ({error_pos[0]}, {error_pos[1]}) - True: {true_str}, Predicted: {pred_str}\n")
